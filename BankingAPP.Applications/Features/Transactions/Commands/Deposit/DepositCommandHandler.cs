...
namespace BankingAPP.Applications.Features.Transactions.Commands.Deposit
{
    public class DepositCommandHandler : IRequestHandler<DepositCommand, TransactionHistoryDto>
    {
        ...
        {
            Log.Information("Starting deposit for account {AccountId} with amount {Amount}",
            ...
            {
                Log.Error(ex, "Error occurred while processing deposit for account {AccountId}", account.Id);
                throw;
            }
